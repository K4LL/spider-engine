...
using namespace spider_engine::hub;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow) {
    ...
    std::unique_ptr<ProjectManager> projectManager = std::make_unique<ProjectManager>();
    
    HubHomePage homePage(
        &coreEngine,
        projectManager.get()
    );
    ...
    auto updateLoop = [&]() {
        renderer.beginFrame();
        renderer.clear(DirectX::Colors::Black);
        renderer.beginImGui();
        
        //// Pega viewport principal
        //ImGuiViewport* vp = ImGui::GetMainViewport();
        //ImVec2 safePos = ImVec2(vp->Pos.x + 50, vp->Pos.y + 50);
        //ImVec2 safeSize = ImVec2(vp->Size.x / 2.0f, vp->Size.y - 100);

        //// Força posição, tamanho e viewport
        //ImGui::SetNextWindowViewport(vp->ID);
        //ImGui::SetNextWindowPos(safePos, ImGuiCond_Always);
        //ImGui::SetNextWindowSize(safeSize, ImGuiCond_Always);

        //// Abre preso na janela principal, sem usar configs salvas
        //ImGui::Begin("Style Editor (Forced)", nullptr);
